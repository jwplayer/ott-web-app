...
import { generateEpisodeJSONLD } from '@jwp/ott-common-next/src/utils/structuredData';
import { getEpisodesInSeason, getFiltersFromSeries } from '@jwp/ott-common-next/src/utils/series';
import { createVideoMetadata } from '@jwp/ott-common-next/src/utils/metadata';
import { formatSeriesMetaString } from '@jwp/ott-common-next/src/utils/formatting';
import { buildLegacySeriesUrlFromMediaItem, mediaURL } from '@jwp/ott-common-next/src/utils/urlFormatting';
import { VideoProgressMinMax } from '@jwp/ott-common-next/src/constants';
...
import { useSeries } from '@jwp/ott-hooks-react-next/src/series/useSeries';
import { useEpisodes } from '@jwp/ott-hooks-react-next/src/series/useEpisodes';
import { useSeriesLookup } from '@jwp/ott-hooks-react-next/src/series/useSeriesLookup';
import { useNextEpisode } from '@jwp/ott-hooks-react-next/src/series/useNextEpisode';
...
import { useFirstEpisode } from '@jwp/ott-hooks-react-next/src/series/useFirstEpisode';
...
const MediaSeries: ScreenComponent<PlaylistItem> = ({ data: seriesMedia }) => {
  ...
  const goBack = useCallback(() => {
    router.push(createURLFromLocation(location, { e: episode?.mediaid, r: feedId || '' }));
  }, [router, location, episode, feedId]);

  const handleComplete = useCallback(async () => {
    router.push(createURLFromLocation(location, { e: (nextItem || episode)?.mediaid, r: feedId || '', play: nextItem ? '1' : '0' }));
  }, [router, location, nextItem, episode, feedId]);

  const handleInlinePlay = useCallback(async () => {
    if (!episode) {
      router.push(createURLFromLocation(location, { e: firstEpisode?.mediaid, r: feedId || '', play: '1' }));
    }
  }, [router, location, firstEpisode, feedId, episode]);
    ...
    () =>
      playEpisode && (
        <StartWatchingButton
          ...
          item={playEpisode}
          onClick={() => {
            router.push(createURLFromLocation(location, { e: playEpisode.mediaid, r: feedId || '', play: '1' }));
          }}
        />
      ),
    [episodeId, playEpisode, router, location, feedId],
  );
