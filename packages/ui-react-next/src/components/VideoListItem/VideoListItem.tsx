...
import { formatDurationTag, formatLocalizedDateTime, formatSeriesMetaString } from '@jwp/ott-common-next/src/utils/formatting';
import { isLiveChannel, isSeries } from '@jwp/ott-common-next/src/utils/media';
import { MediaStatus } from '@jwp/ott-common-next/src/utils/liveEvent';
...
function VideoListItem({ onHover, progress, activeLabel, item, url, loading = false, isActive = false, isLocked = true }: VideoListItemProps): JSX.Element {
  ...
  return (
    <Link role="button" href={url} className={styles.listItem} onMouseEnter={onHover} tabIndex={0} data-testid={testId(title)}>
