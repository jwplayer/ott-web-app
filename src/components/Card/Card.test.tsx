...
describe('<Card>', () => {
  ...
  it('renders the image with the image prop when valid', () => {
    const { getByAltText } = render(<Card title="This is a movie" duration={120} onClick={() => ''} image="http://movie.jpg" />);

    expect(getByAltText('This is a movie')).toHaveAttribute('src', 'http://movie.jpg');
  });
  ...
  it('uses the fallback image when the image fails to load', () => {
    const { getByAltText } = render(
      <Card title="This is a movie" duration={120} onClick={() => ''} image="http://movie.jpg" fallbackImage="http://fallback.jpg" />,
    );

    fireEvent.error(getByAltText('This is a movie'));

    expect(getByAltText('This is a movie')).toHaveAttribute('src', 'http://fallback.jpg');
    expect(getByAltText('This is a movie')).toHaveStyle({ opacity: 0 });
